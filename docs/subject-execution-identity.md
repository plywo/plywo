...
Customer execution does not use the shared account home as writable state. For each active worktree Plywo creates a subject-owned `tmp/plywo/home` and sets `HOME` to that path. `USER` and `LOGNAME` remain the declared subject user. Baseline and candidate therefore cannot communicate through package-manager caches, dotfiles, or other mutable home-directory state.
